...
public class AccountController {
                    ...
                    content = @Content(
                            ...
                            examples = @ExampleObject(
                                    ...
                                    value = "{\"balance\": \"2000\"," +
                                            ...
                                            "\"user_id\": \"12\"}",
                                    description = "Warning: user_id must be valid"
                            )
                    )
            )
    )
    @ApiResponses(
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ViewAccountDto.class),
                            examples = @ExampleObject(
                                    value = "{\"id\": \"1\"," +
                                            "\"balance\": \"2000\"," +
                                            "\"account_type\": \"STANDARD\"," +
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    description = "Details to update the account",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = UpdateAccountDto.class),
                            examples = @ExampleObject(
                                    name = "update account",
                                    value = "{\"balance\": \"3000\"}",
                                    description = "Only balance can be updated"
                            )
                    )
            ),
            parameters = @io.swagger.v3.oas.annotations.Parameter(
